-- =====================================================
-- CLEAN FRESH APPI SCHEMA SCRIPT
-- =====================================================
-- This script completely cleans the database and creates
-- a fresh APPI schema from scratch
-- =====================================================
...
-- STEP 1: BACKUP CRITICAL DATA (if any exists)
-- Create backup schema for any important data
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_users' AND table_schema = 'public') 
       AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_users' AND table_schema = 'backup_20241220') THEN
        EXECUTE 'CREATE TABLE backup_20241220.admin_users AS SELECT * FROM admin_users';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_sessions' AND table_schema = 'public') 
       AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_sessions' AND table_schema = 'backup_20241220') THEN
        EXECUTE 'CREATE TABLE backup_20241220.admin_sessions AS SELECT * FROM admin_sessions';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_log_entries' AND table_schema = 'public') 
       AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_log_entries' AND table_schema = 'backup_20241220') THEN
        EXECUTE 'CREATE TABLE backup_20241220.audit_log_entries AS SELECT * FROM audit_log_entries';
    END IF;
END $$;
...
-- STEP 3: DROP ALL EXISTING VIEWS
...
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT tgname FROM pg_trigger t JOIN pg_class c ON t.tgrelid = c.oid JOIN pg_namespace n ON c.relnamespace = n.oid WHERE n.nspname = 'public' AND NOT t.tgisinternal) LOOP
        EXECUTE 'DROP TRIGGER IF EXISTS ' || quote_ident(r.tgname) || ' ON public.* CASCADE';
    END LOOP;
END $$;

-- STEP 6: DROP ALL EXISTING TYPES
...
-- STEP 7: DROP ALL EXISTING SEQUENCES
...
SELECT 
    'CLEAN STATE VERIFICATION' as check_type,
    COUNT(*) as remaining_objects,
    CASE 
        WHEN COUNT(*) = 0 THEN 'Database is clean - ready for fresh schema'
        ELSE 'Some objects remain - check manually'
    END as status
FROM (
    ...
    UNION ALL
    SELECT typname FROM pg_type t JOIN pg_namespace n ON t.typnamespace = n.oid WHERE n.nspname = 'public' AND t.typtype IN ('e', 'c', 'd')
) as remaining_objects;

-- STEP 10: CREATE FRESH APPI SCHEMA
...
-- Create internal_reports table (for member portal)
...
-- Create draft_declarations table (for member portal)
...
-- Create training_materials table (for member portal)
...
-- Create toolkits table (for member portal)
...
-- Create party_engagement table (for member portal)
...
-- Create reform_progress table (for member portal)
...
-- Create scheduled_activities table (for member portal)
...
-- Create admin_users table (for admin dashboard)
...
('admin@appi.org', 'APPI Administrator', crypt('admin123', gen_salt('bf')), 'admin');
...
SELECT 
    'FRESH APPI SCHEMA COMPLETE' as status,
    COUNT(*) as total_tables,
    'Clean APPI database ready for use' as note
FROM information_schema.tables 
WHERE table_schema = 'public';

-- STEP 15: SUMMARY OF CREATED TABLES
SELECT 
    'CREATED TABLES' as category,
    table_name,
